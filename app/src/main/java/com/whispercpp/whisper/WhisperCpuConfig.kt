...
object WhisperCpuConfig {
    val preferredThreadCount: Int
        // The Snapdragon 8 Elite is all-big-core (2 prime + 6 performance, no little cores), so the
        // frequency-binning heuristic below under-counts it (it drops the largest bin and returns ~2).
        // Whisper scales to roughly the physical big-core count, so take the larger of the heuristic
        // and most of the cores, capped at 6 to leave headroom for the UI + audio capture threads.
        get() = maxOf(CpuInfo.getHighPerfCpuCount(), Runtime.getRuntime().availableProcessors() - 2)
            .coerceIn(4, 6)
}
